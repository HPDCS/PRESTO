...
//			/* DEBUG :: TTMAP_FIND could return NULL */
//			if (customerPtrApp == NULL)
//				continue;
//			/* DEBUG :: customerPtrApp->C_LAST causes SIGSEGV */
...
/*	long i, c = 0, o = 0;
	PROTECT;
	long idx = TM_SHARED_READ(idxOrders[0]);
	UNPROTECT(1);
	if (idx == -1)
		goto stock_level_tx_end;
	PROTECT;
	long orders[20] = { [0 ... 19] = -1 };
	UNPROTECT(0);
	for (; o<1000 && c<20; o++) {
		i = (idx-o >= 0) ? idx-o : 1000+idx-o;
		PROTECT;
		key = TM_SHARED_READ(lastOrders[i]);
		UNPROTECT(1);
		if (key == -1)
			continue;
		if (PROTECT_BLOCK(1, TMMAP_CONTAINS(districtPtr->orderMap, key), long)) {
			orders[c] = key;
			c++;
		}
	}
...
*/
	...
	for(key=( PROTECT_BLOCK(1,TM_SHARED_READ(districtPtr->D_NEXT_O_ID),stm_word_t) - 20 );
