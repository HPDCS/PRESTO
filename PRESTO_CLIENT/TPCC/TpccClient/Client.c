...
long createInstruction(char* buffer) {
	...
	if (tx_id == 1) {
		...
		sprintf(buffer, "%ld;%ld;%ld;%ld;%ld\0", prio, tx_id, id_w, id_d, id_c);
	...
	if (tx_id == 2) {
		...
		sprintf(buffer, "%ld;%ld;%ld;%ld;%ld;%s;%f\0", prio, tx_id, id_w, id_d, id_c, " ", h_am);
	...
	if (tx_id == 4) {
		...
		sprintf(buffer, "%ld;%ld;%ld;%ld;%ld\0", prio, tx_id, id_w, id_d, tr);
	...
	if (tx_id == 5) {
		...
		sprintf(buffer, "%ld;%ld;%ld;%ld\0", prio, tx_id, id_w, car);
...
int main(int argc, char* argv[]) {
	...
	if (argc < 7) {
		fprintf(stderr,"[Error] Usage: %s  server_address  port_number  group_number  group_size  txs_x_proc  arrival_rate_x_proc(txs/sec.)\n", argv[0]);
		exit(0);
	}
	...
	if ((input_file = fopen("inputs.txt","r")) == NULL) {
		fprintf(stderr,"[Error] FOpen: unable to open the inputs file.\n");
		free(percentage);
		exit(0);
	}
